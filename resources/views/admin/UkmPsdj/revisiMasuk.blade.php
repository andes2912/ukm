	...
	<div class="col-md-12">
		<article class="widget">
			...
			<div class="widget__content table-responsive">
				...
				<tbody>
					@foreach($revisimasukPsdj as $revisimasukPsdj)
					<tr class="spacer"></tr>
					<tr class="spacer">

						<td>
							<p class="post_info"> {{$revisimasukPsdj->id}} </p>
						</td>
						<td>
							<p >{{ $revisimasukPsdj->title }}</p>
													<p class="post__info"> Pengirim : {{ Auth::user()->name}}</p>
						</td>
						<td>
							<p class="post__date">{{ $revisimasukPsdj->created_at }}</p>
						</td>
						<td>
							<p class="post__info">{{ $revisimasukPsdj->created_at->diffForHumans() }}</p>
						</td>
						
						<div class="dropdown">
							<td>
								<a href="{{ route('DownloadKmhPsdj.download', $revisimasukPsdj->id) }}" class="btn red">Lihat</a>								
							</td>
						</div>
					</tr>
						@endforeach
